...
export function BottomNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-xl flex justify-around items-center pt-2 pb-5 px-2 shadow-[0_-10px_40px_rgba(0,0,0,0.08)] rounded-t-3xl z-50">
      ...
      <div className="relative w-16 h-12 flex justify-center">
        <button 
          ...
          className="absolute -top-10 w-[68px] h-[68px] bg-gradient-to-br from-primary to-primary-dark text-white rounded-full flex flex-col justify-center items-center shadow-[0_10px_30px_rgba(0,200,83,0.45)] border-4 border-[#F0F2F5] transition-all hover:scale-110 active:scale-95 group"
        >
...
function NavItem({ icon: Icon, label, isActive, onClick }: { icon: any, label: string, isActive: boolean, onClick: () => void }) {
  return (
    <button 
      onClick={onClick}
      className={cn(
        "flex flex-col items-center gap-1 w-16 p-1.5 rounded-2xl transition-all relative",
        isActive ? "text-primary" : "text-text-muted hover:text-primary hover:-translate-y-1"
      )}
    >
      <Icon size={isActive ? 22 : 20} className="transition-all" />
